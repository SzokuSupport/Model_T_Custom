...
public class DefaultB2BUnitService implements B2BUnitService<B2BUnitModel, B2BCustomerModel>
{
	...
	{
		return getBranchHierarchy(unit);
	}
	...
	protected Set<B2BUnitModel> getB2BHierarchyUnits(Set<B2BUnitModel> units)
	{
	...
	protected Set<B2BUnitModel> getBranchHierarchy(final B2BUnitModel parent)
	{
		if(LOG.isDebugEnabled())
		{
			LOG.debug("Using performance getBranchHierarchy method.");
		}

		final Set<B2BUnitModel> result = new HashSet<B2BUnitModel>();
		result.add(parent);
		Set<B2BUnitModel> toQuery = Collections.singleton(parent);
		do
		{
			// grab whole next layer
			final Set<B2BUnitModel> currentLevel = new HashSet<B2BUnitModel>(getB2BHierarchyUnits(toQuery));
			// cycle safe
			currentLevel.removeAll(result);
			// add to result
			result.addAll(currentLevel);
			toQuery = currentLevel;
		}
	...
	public void updateBranchInSession(final Session session, final UserModel currentUser)
	{
		if (currentUser instanceof B2BCustomerModel)
		{
			...
			if(LOG.isDebugEnabled())
			{
				LOG.debug("Updating branch for user: "+currentUser+" - rootunit: "+branchInfo[0]+" - branch: "+branchInfo[1]+" - unit: "+branchInfo[2]);
			}
		}
		else
		{
			if(LOG.isDebugEnabled())
			{
				LOG.debug("currentUser: "+currentUser+" is not an instance of B2BCustomerModel. Will not update branch in session.");
			}
		}
	}
