        ...
        AuthenticationServiceInterface.prototype.logout = function() {
            ...
            return crossFrameEventService.publish(EVENTS.LOGOUT, {}).finally(
                function(storage) {
                    ...
                    //$log.info("before cookies:" + document.cookie);
                    var expires = new Date(0).toUTCString();
                    document.cookie = 'SmarteditSSOToken=; path=/; expires=' + expires;

                    //$log.info("after cookies:" + document.cookie);

                    //redirect to idp logout
                    $log.info("-----------------sso redirect to idp logout---------------");
                    var loc = window.location;
                    var server = loc.protocol + '//' + loc.hostname + (loc.port ? ':' + loc.port : '');
                    window.location.href = server + "/oidcsinglesignon/openid/logout";
