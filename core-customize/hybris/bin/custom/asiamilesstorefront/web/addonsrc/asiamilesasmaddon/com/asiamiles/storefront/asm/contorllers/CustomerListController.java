...
public class CustomerListController extends AbstractSearchPageController
{
			...
			@RequestParam(value = "query", required = false) final String query)
	{
		...
		catch (final Exception exp)
		{
			final String errorMessage = String.format("sessionId=[%s],agentNo=[%s],service=[%s],action=[%s],error message =[%s]",
					getSessionId(), getAgentNo(), "CustomerListController", "listPaginatedCustomers", exp.getMessage());
	...
	{
		final AssistedServiceSession session = assistedServiceFacade.getAsmSession();
