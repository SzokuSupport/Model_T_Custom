...
<input type="text" name="needSmsDeliveryInfo" hidden=true,
	value="${smsDeliveryInfo.needSmsDeliveryInfo}" id="SMSNeedOther" />
	...
	<div class="row step-one__SMS--right">
		<div class="col-lg-6 col-sm-6">
			<div class="dropdown-group__checkOut J-country-code">
				<label for="capacity" class="dropdown-label">Country / Region code</label>
				<a class="dropdown-selected J-selectTrigger" aria-hidden="false" tabindex="0">
					...
					<input class="J-select-input" name="smsDeliveryCountryCode" type="hidden" id='capacity'/>
				...
				<!-- /desktop dropdown -->
				<!-- mobile dropdown -->
				<select class="dropdown-selected-control--mobile J-selectedControlMobile" name="title" id="otherSmsCuntryListBoxPhone">
					<c:forEach items="${phoneCountryCode}" var="phoneCountry">
						<c:if test="${recipientInfo.countryCode==phoneCountry.phoneCode}">
		...
		<div class="col-lg-6 col-sm-6">
			<div class="input-component J-input-transform">
				<input type="text" id="phoneOther" name="smsDeliveryPhoneNo" value="${smsDeliveryInfo.phoneNo}" />
