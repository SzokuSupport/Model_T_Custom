					...
					<c:if test="${product.dispalyHomePage}">
						...
						<div class="item-list">
							<a class="product-card" href="${productUrl}" event_component="promotion_nav" event_value="${fn:escapeXml(title)}:${fn:escapeXml(product.name)}" event_position="${looper.index}">
								<div class="product-img J-product-card-img ImgToBg whiteBg">
			               ...
			               <div class="product-sale">
			                   ...
			                   <c:if test="${not empty product.promotionPrice}">
					             ...
					             <c:if test="${not empty promotionPrice}">
				                   ...
				                   <div class="product-sale-discount J-see-all-discount" <c:if test="${!empty product.discountRows}"> data-url="/product/${product.code }/tooltip" tabindex='0'</c:if>>
				                      <c:if test="${!empty product.discountRows}">
				                        <p class="title">
				                        	<c:choose>
				                              <c:when test="${customerAccountBalance ge 0}">
				                               	<spring:theme code="text.product.list.page.product.discount.all"/>
				                              </c:when>
				                              <c:otherwise>
