...
<div class="popup">
    <div class="discount promotion">
        <p class="discount-title font-light"><spring:theme code="text.cart.page.slider.promotion.popup.title"/></p>
        <p class="title-below"><spring:theme code="text.cart.page.slider.promotion.popup.description"/></p>
        <div class="discount-price">
            <div class="discount-price__top">
                ...
                <span class="font-bold"><fmt:formatNumber value="${totalPriceAfterDiscount}"  maxFractionDigits="0"/></span>
            </div>
            <div class="discount-price__bottom">
                <span><spring:theme code="text.cart.page.slider.promotion.popup.origin.price1"/></span>
                <svg class="icon-yellow" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                     width="40" height="48" viewBox="0 0 40 48">
                    <g fill="none" fill-rule="evenodd">
                        <path fill="#FACF00"
                ...
                <span class="font-SemiBold"><fmt:formatNumber value="${originPrice}" maxFractionDigits="0"/><spring:theme code="text.cart.page.slider.promotion.popup.origin.price2"/> </span>
            </div>
        </div>
        <div class="main">
            <c:forEach var="discount" items="${discountRows}">
                <c:set var="type" value="${discount.discountType}"></c:set>
                <div class="section">
                    <c:choose>
                        <c:when test="${type eq 'OTHER' }">
<!--                             <svg class="icon-tooltip" xmlns="http://www.w3.org/2000/svg" width="24" height="24" -->
<!--                                  viewBox="0 0 24 24"> -->
<!--                                 <g fill="none" fill-rule="evenodd"> -->
<!--                                     <path fill="#272F38" -->
...
<!--                                     <path fill="#272F38" fill-rule="nonzero" -->
...
<!--                             </svg> -->
                            ...
                            <c:choose>
                                <c:when test="${language eq 'zh'}">
                                    <c:set var="startTime"><fmt:formatDate value="${discount.startTime}"
                                                                           pattern="yyyy年MM月dd日"/></c:set>
                                    <c:set var="endTime"><fmt:formatDate value="${discount.endTime}"
                                                                         pattern="yyyy年MM月dd日"/></c:set>
                                </c:when>
                                ...
                                <c:when test="${language eq 'ja'}">
                                    <c:set var="startTime"><fmt:formatDate value="${discount.startTime}"
                                                                           pattern="yyyy年 MM月 dd日 "/></c:set>
                                    <c:set var="endTime"><fmt:formatDate value="${discount.endTime}"
                                                                         pattern="yyyy年 MM月 dd日 "/></c:set>
                                </c:when>
                                <c:when test="${language eq 'ko'}">
                                    <c:set var="startTime"><fmt:formatDate value="${discount.startTime}"
                                                                           pattern="yyyy년 MM월 dd일 "/></c:set>
                                    <c:set var="endTime"><fmt:formatDate value="${discount.endTime}"
                                                                         pattern="yyyy년 MM월 dd일 "/></c:set>
                                </c:when>
                                <c:otherwise>
                                    <c:set var="startTime"><fmt:formatDate value="${discount.startTime}" type="both"
                                                                           pattern="d MMM yyyy "/></c:set>
                                    <c:set var="endTime"><fmt:formatDate value="${discount.endTime}" type="both"
                                                                         pattern="d MMM yyyy "/></c:set>

                                </c:otherwise>
                            </c:choose>
                            <p class="time font-bold"><spring:theme
                                    code="text.cart.page.slider.promotion.popup.promotion_period"/> ${startTime} - ${endTime}</p>
                            <a class="terms-conditions" href="${discount.termsAndCondition}"><spring:theme
                                    code="text.cart.page.slider.promotion.popup.terms.and.condition"/></a>
                        </c:when>
                    </c:choose>
                </div>
            </c:forEach>
...
<!--                             </svg> -->
    ...
    <svg class="icon-close J-close-tooltip" width="24px" height="24px" viewBox="0 0 24 24" version="1.1"
